...
class Type {
  ...
  public:
    ...
    int isPointer(){
        return ptr_to_arr || (arr_sizes.size()==0 && ptr_cnt);
    }

    int sizeOf(){
        if(isTypeStruct()){
            return structSize(basic_type_name);
        }
        if(ptr_to_arr || arr_sizes.size()==0){return 4;}
        // isArray()
        int tmp = 1;
        for(auto s: arr_sizes){
            tmp *= s;
        }
        Type t = Type(this);
        t.arr_sizes.clear();
