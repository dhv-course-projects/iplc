...
class st_entry {
  ...
  public:
    ...
    void genCode(){
        // expected that only called for st_entry_type kFunction
        currentST = local_symbol_table;
        seq_node->genCode();
        if(type->getTypeName() == "void"){ // our grammar doesn't have "return;". non-void functions rely on return_astnode to produce leave, ret
            std::cout << "\tleave\n\tret\n";
        }
    }
};
...
class SymbolTable {
  private:
    ...
    static bool myComp(const std::pair<Type, int> &a, const std::pair<Type, int> &b) {
        return a.second > b.second;
    }
  public:
    ...
    std::vector<std::pair<Type, int>> getStructMembers() {
        std::vector<std::pair<Type, int>> members;
        st_entry *entry;
        for (auto it: table) {
            entry = it.second;
            members.push_back({Type(entry->getType()), entry->getOffset()});
        }
        std::sort(members.begin(), members.end(), myComp);
        return members;
    }
    ...
    int getParamStackSize() {
        ...
        for (auto it: table) {
            entry = it.second;
            if (entry->getScope() == kParameter){
                if(entry->getType()->isArray()){size += 4;}
                else{size += entry->getWidth();}
            }
        }
        return size;
    }
    ...
    void genCode(){
        for(auto it: table){
            if(!it.second->isTypeFunction()){continue;}
            std::cout << "\t.text\n\t.globl\t" << it.second->getName() << "\n\t.type\t" << it.second->getName() << ", @function\n";
            std::cout << it.second->getName() << ":\n\tpushl\t%ebp\n\tmovl\t%esp, %ebp\n";
            
            SymbolTable* localST = it.second->getLocalSymbolTable();
            int tmp = 0;
            for(auto localVar : localST->table){
                if(localVar.second->getOffset() < tmp){tmp = localVar.second->getOffset();}
            }
            tmp = -tmp;

            std::cout << "\tsubl\t$" << tmp << ", %esp\n"; //subl	$20, %esp etc
            it.second->genCode();
            std::cout << "\t.size\t" << it.second->getName() << ", .-" << it.second->getName() << "\n";
        }
        std::cout << "\t.section\t.rodata\n\t.align\t4\n";
        for(size_t i=0; i<ro_consts.size(); ++i){
            std::cout << ".LC" << i << ":\n\t.string\t" << ro_consts[i] << "\n" ;
        }
    }
};
